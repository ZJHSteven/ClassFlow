...
## 当前结论（必须最新）
- 现状：`2026-04-22` 正在继续只读排查写入产物失败的两个新假设：一是 OSS 上传与 Worker 写入同时占用很小的上行带宽导致超时，二是 `mihomo` 代理链路在失败窗口存在明确超时或路由异常。
- 正在做：还原北京时间 2026-04-21 21:20 到 21:50 的 OSS 上传、Worker 写入和失败重试时间线，并读取同窗口 `mihomo` 日志。
- 下一步：对比并发上传线路数量、产物体积、OSS 音频体积和代理日志后，再讨论是否需要系统级 QoS / 总上传限速，或应用内并发保护。
