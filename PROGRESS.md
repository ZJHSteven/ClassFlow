...
- 已完成：`mihomo` 日志确认 OSS 上传走 `DIRECT`，Worker 写入走 `Kuromis[香港 01]`；失败最早窗口 `21:29:18~21:29:24` 同时出现 `wlp3s0` AP 漫游/重连、认证超时、TUN 默认接口变化、`cloudflared` QUIC `timeout: no recent network activity`。
- 已完成：`21:39`、`21:41`、`21:46`、`21:54` 附近继续出现 Wi-Fi AP 切换/掉 carrier，说明当晚网络底层确实不稳。`speedtest-tracker` 21:30 有外部连接，但数据库显示测速失败在 `ConfigurationError`，不是持续打满带宽的主证据。
- 下一步：若要治理，优先级建议为：先处理 Wi-Fi 漫游/不稳定和半小时 speedtest 干扰，再考虑系统级上行 QoS / `tc` 整形，最后补应用内 Worker 产物写入并发与更长退避重试。
