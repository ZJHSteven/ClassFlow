...
### 执行步骤
1. 正在做：读取 systemd 后端日志、SQLite 任务表与事件日志，筛出今天所有失败后成功的任务，记录任务 ID、课程、阶段、错误文本、失败时间与重试成功时间。
2. 待做：审查 `WorkerArtifactStore`、`sync_course_artifacts` 与 Worker 私有产物接口的现有实现，确认写入阶段实际会连续写哪些对象、每个对象是否有重试、哪些失败会中断整任务。
3. 待做：复盘相关 git 历史，找出此前“协助宠物/写入产物”链路的改动点，并对照今天错误是否命中同一边界。
4. 待做：做只读或临时探针压测：验证后端到 Worker 私有产物接口的 `PUT/DELETE` 延迟、状态码、Access 头、代理路径稳定性；探针对象使用临时路径并清理，不改业务数据。
5. 待做：运行现有后端与 Worker 测试，确认当前代码层面的产物写入、Access 重定向诊断、公开产物读取测试仍全绿。
6. 待做：汇总根因判断、复现概率、直接证据与下一步修复建议；若确认需要改代码，再另起修复计划。

### 当前状态
- 正在做：本轮按“只查日志和测试，不改业务代码”的边界执行。
