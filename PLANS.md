...
### 执行步骤
1. 正在做：审查 `worker.rs`、`pipeline.rs`、`artifacts.rs`、SQLite 仓储与现有 API 测试，定位上传检查点、转写检查点、产物写入错误传播的当前实现。
2. 待做：读取真实数据库与系统级后端日志，按课程名、日期、阶段、错误文本筛出“医学人文英语”和最近三节“毛泽东思想”失败任务，确认失败发生时间、任务 ID、检查点字段和最新错误。
3. 待做：修改重试策略：如果失败阶段在转写及之前，且保存的是 `oss://` 临时 URL，就不能跨多天盲跳上传；应在过期窗口外清空旧上传检查点并重新上传音频。
4. 待做：增强 Worker 产物写入 404 / HGP404 的错误上下文，至少包含产物路径、HTTP 状态、响应体、请求基准 URL，避免只显示无法定位来源的短码。
5. 待做：新增或更新后端测试，覆盖临时 OSS 过期重试、未过期重试仍可复用上传检查点、产物写入 404 错误上下文。
6. 待做：运行后端格式化、静态检查、单元测试与集成测试；若需要部署，继续执行 release 构建、重启系统级 `classflow-backend.service` 与真实任务重试验收。

### 当前状态
- 正在做：已经确认代码中存在“只要 `uploaded_source_url` 非空就跳过上传”的路径，尚未看到对 `oss://` 临时对象有效期的判断。
