...
13. 进行中：按上线前最终口径收口前端与部署：彻底移除 `useReducedMotion` 降级逻辑、确保“任务运行中的下载/上传进度与速率”能在前端持续可见，并在验证通过后同步部署前后端。
14. 新增收口项：修复后端下载进度采集的时序错误，避免 `aria2c` 进度只在进程退出后才一次性写库，导致任务运行中前端始终显示 `0`。
